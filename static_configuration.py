...
static_config = {
    ...
    "initial_wc_distribution": lambda resid_wc, wc_10, wc_40, sat_wc, profile: np.concatenate((np.linspace(4*resid_wc, wc_10, 10), np.linspace(wc_10, wc_40, 30), np.linspace(wc_40, 0.9*sat_wc, len(profile)-40))),
    "initial_conc": 0,
    "initial_conc_distribution": lambda initial_conc, profile: np.full(len(profile), initial_conc), # finish this for starting stage 
    "croot_max": 0,
    "active_uptake_amount" : 0.3, # can be a series, the amount is per hour
    ...
    "daily_et": 0.2,  # output flux of water by evaporation or by plant uptake per day
